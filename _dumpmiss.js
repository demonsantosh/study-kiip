...
Object.keys(MD).forEach((id)=>{ if(id.indexOf("@")!==-1) return;
  ...
  for(let i=0;i<ls.length;i++){
    ...
    let srcEn="", sne="";
    if(em){ srcEn=em[1]; const ni=srcEn.search(/\b(?:NE|NP|Nepali)\s*[:：]/i); if(ni>=0){ sne=srcEn.slice(ni).replace(/^.*?(?:NE|NP|Nepali)\s*[:：]\s*/i,"").trim(); srcEn=srcEn.slice(0,ni).trim(); } if(!sne){ let k2=j+1; while(k2<ls.length && !ls[k2].trim()) k2++; const mn=(ls[k2]||"").trim().match(/^🇳🇵\s*(.*)$/); if(mn) sne=mn[1].trim(); } }
...
const all=missWords.concat(missSents).map(w=>({ko:w.ko,en:w.en,need:LANGS.filter(l=> l==="ne"?!hasNe(w.ko):!(data[w.ko]||{})[l])}));
require("fs").writeFileSync("_miss.json", JSON.stringify(all));
console.log("dumped", all.length);
