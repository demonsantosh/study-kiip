...
Object.keys(MD).forEach((id)=>{ if(id.indexOf("@")!==-1) return;
  ...
  for(let i=0;i<ls.length;i++){
    ...
    parts.forEach((s)=>{
      ...
      if(parts.length===1 && sne && /[ऀ-ॿ]/.test(sne)) srcNe.add(s);
...
// clean heuristic: no stray digits adjacent to hangul, no replacement char, no lone bracket
function isClean(k){
  if(/�/.test(k)) return false;
  if(/[0-9]/.test(k)) return false;              // any digit -> likely OCR contamination for these
  if(/[（(]\s*$|^\s*[)）]/.test(k)) return false;
  const kr=(k.match(/[가-힣]/g)||[]).length;
  if(kr < 6) return false;                        // too short/fragmentary
  if(kr/k.replace(/\s/g,"").length < 0.75) return false; // mostly symbols
  return true;
}
const cleanMiss=all.filter(x=>isClean(x.ko));
fs.writeFileSync("_cleanmiss.json", JSON.stringify(cleanMiss,null,0));
console.log("missing-any:",all.length,"words:",missWords.length,"sents:",missSents.length,"| clean-missing:",cleanMiss.length);
