...
const BARE_PARTICLE = /^(은|는|을|를|와|과|의|및|랑|부터|까지|에서|에게|으로|이나|든지|처럼)$/;
...
// Staging: chapters listed in staging.json are translated in the background but hidden
// from the learner UI (mergeLessons in app.js skips hidden chapters). nextwords still
// translates them because it reads LESSON_VOCAB / LESSON_MD directly.
try {
  const staging = JSON.parse(fs.readFileSync(path.join(__dirname, "staging.json"), "utf8"));
  (staging.hidden || []).forEach((id) => { if (META[id]) META[id].hidden = true; });
  const hid = (staging.hidden || []).filter((id) => META[id]);
  if (hid.length) console.log("staged (hidden until translated):", hid.join(", "));
} catch (e) { /* no staging file — nothing hidden */ }
